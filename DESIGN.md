...
- Make seeking discoverable with a visible native range handle over the waveform, a 44px hit region, and matching waveform/handle endpoints. Dragging pauses the note and shows an out-of-flow timestamp above the handle; release resumes only a previously playing note. Keyboard arrows move one second, Page keys ten seconds, Home/End reach either endpoint. Cancel restores the pre-drag position; app switching ends the gesture without surprise autoplay. On narrow bubbles or enlarged text, reflow duration/speed below Play/waveform rather than shrinking targets or overflowing.
