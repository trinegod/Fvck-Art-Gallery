...
- The play icon, waveform and duration share one horizontal centerline. Draw playback position from the real media clock on animation frames while actively playing; sparse `timeupdate` events alone make the marker jump. Do not slow audio or use CSS easing that lags behind seeks. Suspend frame work when paused, buffering, hidden or disposed. This is functional progress feedback, not decorative motion; keep it accurate with reduced motion too.
