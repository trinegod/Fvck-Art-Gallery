...
3. **Hierarchy:** give the screen one primary purpose. Group related controls through spacing and labels, use sensible reversible defaults, and make the principal action distinct. Judge the actual task; there is no universal maximum count of navigation items or fields.
   **Owner alignment preference:** check optical centering and balanced spacing across mobile and desktop. Related icons, graphic centerlines and short metadata should align, including loading states. Measure the actual visible elements rather than only their outer containers. Do not center paragraphs or sacrifice readable text, 44px targets or reflow merely to create symmetry.
...
- **Navigation:** four section links remain the top level. A resolved mobile conversation has a focused header without the global dock; the inbox and browsing pages retain the owner-approved icon-only dock with accessible names. Removing visible labels must also reduce the actual dock and reserve height, not merely hide text. Test 320px and 390px layouts, desktop breakpoint, browser back, long content, and bottom clearance. The mobile keyboard must not hide a focused composer or its action. See the [chat-space audit](../audits/2026-09-07-mobile-chat-space.md) for measured evidence and outstanding real-device checks.
