...
export default function MobileAppNavigation({ hidden = false }: MobileAppNavigationProps) {
  ...
  return (
    <nav
      ...
      className="nodeine-mobile-navigation fixed inset-x-3 bottom-[max(.75rem,env(safe-area-inset-bottom))] z-40 mx-auto grid max-w-sm grid-cols-4 gap-1 rounded-[1.1rem] border border-white/12 bg-zinc-950/95 p-1 shadow-[0_12px_40px_rgba(0,0,0,.55)] backdrop-blur-2xl lg:hidden"
    >
      {appNavigationItems.map((item) => {
        ...
        return (
          <Link
            ...
            className="nodeine-action nodeine-nav-primary relative flex min-h-11 min-w-11 items-center justify-center rounded-[.8rem] px-1 text-zinc-400 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-300"
          >
